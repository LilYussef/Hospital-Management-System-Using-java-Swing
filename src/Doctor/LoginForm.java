...
public class LoginForm extends JFrame implements ActionListener,ItemListener{
    ...
    public LoginForm(){
        ...
        lbl_heading.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 25));
        lbl_heading.setHorizontalAlignment(SwingConstants.CENTER);
        lbl_user=new JLabel("Username");
        lbl_user.setFont(new Font(Font.SANS_SERIF,Font.BOLD,20));
        lbl_password=new JLabel("Password");
        lbl_password.setFont(new Font(Font.SANS_SERIF,Font.BOLD,20));
        ...
        lbl_text2.setFont(new Font(Font.SANS_SERIF,Font.ITALIC,20));
        lbl_text2.setHorizontalAlignment(SwingConstants.CENTER);
        
         
        
        jbtn_submit=new JButton(submit);
            jbtn_submit.setBorderPainted(false);
        jbtn_clear=new JButton(clear);
            jbtn_clear.setBorderPainted(false);
    ...
    public void actionPerformed(ActionEvent e) {
	    ...
	    String str_pswrd=new String(jpf_password.getPassword());
        
        Object obj_source=e.getSource();
        if(obj_source==jbtn_submit ){  
            try {
                Connection con=DatabaseOperation.getConnection();
                PreparedStatement st=con.prepareStatement("SELECT id,password FROM doctor_details WHERE name=? AND password=?");
                ...
                if(rs.next()) {
                    ...
                    if(dbPassword.equals(str_pswrd)){
